...
module.exports = {
  start: async function (api, networkInfo) {
    ...
    if (!networkInfo.testnet) {
      ...
      const runnersUpbalances = await Promise.all(
        members.runnersUp.map((x) => api.derive.balances.all(x[0].toString())),
      );
      ...
      const Council = Container.get(networkInfo.name + 'Council') as mongoose.Model<ICouncil & mongoose.Document>;
      try {
